...
import com.example.studySpringboot0001.entity.StockLog;
...
import static com.example.studySpringboot0001.controller.stockController.*;
...
//        String[] result = {Integer.toString(data0001), Integer.toString(data0002), Integer.toString(data0003)};
    ...
    public void stock_buy(String lowStockName, Integer buy_count) {
        stock_list.get(lowStockName).setOne_stock_money(calc_stock(buy_count, lowStockName));
    }
    ...
    public void scheduled_one_stock_buy() {
        ...
        //검색 - 주식 1주 가격 꺼내오기
        int data1 = stock_list.get("애플").getOne_stock_money();
        int data2 = stock_list.get("삼성전자").getOne_stock_money();
        int data3 = stock_list.get("테슬라").getOne_stock_money();
        //검색 - 가장 작은 주식 찾기
        ...
        System.out.println("현재 가장 저렴한 주식은 "
                +loseStockName
                +"이며, "
                +stock_list.get(loseStockName).getOne_stock_money());
...
//        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        ...
        //로그저장 (항상 시각 기록)
        stockLogs.add(new StockLog(logDate, loseStockName, 1, loseStockPrice));
    }
...
//        }
        ...
        if(index == 0) {
            stock_list.get("애플").setOne_stock_money(calc_stock(1, "애플"));
        }else if (index == 1) {
            stock_list.get("삼성전자").setOne_stock_money(calc_stock(1, "삼성전자"));
        } else if (index == 2) {
            stock_list.get("테슬라").setOne_stock_money(calc_stock(1, "테슬라"));
        } else if (index == 3) {
            stock_list.get("애플").setOne_stock_money(calc_stock(-1, "애플"));
        } else if (index == 4) {
            stock_list.get("삼성전자").setOne_stock_money(calc_stock(-1, "삼성전자"));
